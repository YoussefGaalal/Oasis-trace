    ...
    <div className="flex flex-wrap items-center justify-between gap-4 py-4 px-2">
      <div className="flex items-center gap-3">
        ...
        <select
          ...
          className="bg-neutral-100 border-none rounded-lg px-3 py-2 text-sm font-medium text-brand-primary"
        >
          {perPageOptions.map(opt => (
            <option key={opt} value={opt}>{opt}</option>
          ))}
        </select>
        <span className="text-sm text-neutral-500">of {total} entries</span>
      </div>
