  ...
  const fetchUserContext = async () => {
    try {
      const [animalsRes, alertsRes, geofencesRes] = await Promise.all([
        api.get('/api/animals').catch(() => null),
        api.get('/api/alerts').catch(() => null),
        api.get('/api/geofences').catch(() => null),
      ]);

      const [animals, alerts, geofences] = await Promise.all([
        animalsRes?.ok ? Promise.resolve(animalsRes.data) : Promise.resolve([]),
        alertsRes?.ok ? Promise.resolve(alertsRes.data) : Promise.resolve([]),
        geofencesRes?.ok ? Promise.resolve(geofencesRes.data) : Promise.resolve([]),
      ]);
...
- Total Geofences: ${ctx.totalGeofences}`;
  ...
  const renderSetup = () => (
    <div className={`fixed bottom-24 ${isRtl ? 'left-6' : 'right-6'} z-50 w-[420px] max-w-[calc(100vw-3rem)] bg-brand-light rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[600px]`}>
        ...
        {setupStep === 1 && (
          <div className="text-center">
            <div className="w-20 h-20 bg-brand-primary/10 rounded-full flex items-center justify-center mx-auto mb-6">
              <MaterialSymbol icon="auto_awesome" size={40} className="text-brand-accent" />
            </div>
            ...
            <div className="bg-neutral-100 rounded-2xl p-4 mb-6 text-left">
              <h3 className="font-bold text-brand-primary mb-2 flex items-center gap-2">
                <MaterialSymbol icon="check_circle" size={18} className="text-brand-primary" />
            ...
            <button
              ...
              className="w-full py-3 bg-brand-primary text-white rounded-xl font-bold shadow-lg hover:bg-brand-secondary transition-all"
            >
              {language === 'ar' ? 'احصل على مفتاح API مجاني' : 'Get Free API Key'}
            </button>
            <button
              onClick={skipSetup}
              className="w-full py-3 mt-3 text-neutral-600 text-sm hover:text-brand-primary transition-all"
            >
        ...
        {setupStep === 2 && (
          <div>
            <button
              ...
              className="flex items-center gap-2 text-neutral-600 text-sm mb-4 hover:text-brand-primary"
            >
            ...
            <div className="space-y-3 mb-6">
              <div className="flex gap-3 p-3 bg-neutral-300 rounded-xl">
                <div className="w-8 h-8 bg-brand-primary text-white rounded-lg flex items-center justify-center font-bold text-sm flex-shrink-0">1</div>
              ...
              <div className="flex gap-3 p-3 bg-neutral-300 rounded-xl">
                <div className="w-8 h-8 bg-brand-primary text-white rounded-lg flex items-center justify-center font-bold text-sm flex-shrink-0">2</div>
                <div className="text-sm">
                  <p className="font-semibold text-brand-primary">
              ...
              <div className="flex gap-3 p-3 bg-neutral-300 rounded-xl">
                <div className="w-8 h-8 bg-brand-primary text-white rounded-lg flex items-center justify-center font-bold text-sm flex-shrink-0">3</div>
                <div className="text-sm">
                  <p className="font-semibold text-brand-primary">
          ...
          <div>
            ...
            <div className="mb-4">
              <input
                ...
                placeholder="AIza..."
                className="w-full px-4 py-3 bg-white rounded-xl text-sm border-2 border-neutral-400 focus:border-brand-accent focus:outline-none"
              />
            </div>
            <div className="bg-green-50 rounded-xl p-4 mb-4 text-sm">
              <p className="text-brand-primary font-medium flex items-start gap-2">
            ...
            <button
              ...
              className="w-full py-3 bg-brand-primary text-white rounded-xl font-bold shadow-lg hover:bg-brand-secondary transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
    ...
    <>
      ...
      {isOpen && !showSetup && (
        <div className={`fixed bottom-24 ${isRtl ? 'left-6' : 'right-6'} z-50 w-[420px] max-w-[calc(100vw-3rem)] bg-brand-light rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[600px]`}>
          ...
          {showSettings && (
            <div className="bg-neutral-100 p-4 border-b border-neutral-400">
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="AIza..."
                    className="w-full px-3 py-2 bg-white rounded-lg text-sm border border-neutral-400 focus:outline-none focus:border-brand-accent"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-brand-light">
            {messages.length === 0 && (
              <div className="flex gap-4">
                ...
                <div className="bg-neutral-100 rounded-2xl rounded-tl-sm p-4 max-w-[80%]">
                  ...
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
                    {quickActions.map((action, idx) => (
                      <button
                        ...
                        className="flex items-center justify-center gap-2 px-3 py-2 bg-neutral-300 border border-neutral-400/20 rounded-xl text-xs font-semibold text-brand-primary hover:bg-green-50 transition-all"
                      >
                        <MaterialSymbol icon={action.icon} size={16} className="text-brand-primary" />
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? (isRtl ? 'justify-start' : 'justify-end') : (isRtl ? 'justify-end' : 'justify-start')}`}>
                {msg.role === 'assistant' && !msg.isWelcome && (
                  <div className="w-10 h-10 rounded-full bg-brand-secondary flex-shrink-0 flex items-center justify-center text-white shadow-md mr-2">
                    <MaterialSymbol icon="smart_toy" size={20} weight="fill" />
                  </div>
                )}
                {msg.role === 'assistant' && !msg.isWelcome && (
                  <div className="bg-neutral-100 text-neutral-700 rounded-2xl rounded-tl-sm p-4 max-w-[80%]">
                    <p className="text-sm leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                  </div>
                )}
                {msg.role === 'user' && (
                  <>
                    <div className="bg-brand-primary text-white rounded-2xl rounded-tr-sm p-4 max-w-[80%]">
                      <p className="text-sm leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                    </div>
                    <div className="w-10 h-10 rounded-full bg-green-50 flex-shrink-0 flex items-center justify-center text-green-600 shadow-md ml-2 overflow-hidden">
          ...
          <div className="p-4 bg-brand-light/80 backdrop-blur-xl border-t border-neutral-400">
            <div className="flex gap-2 overflow-x-auto pb-3 mb-3 no-scrollbar">
              {suggestedQuestions.map((q, i) => (
                <button
                  key={i}
                  onClick={() => sendMessage(q)}
                  className="whitespace-nowrap px-3 py-1.5 bg-neutral-100 text-brand-primary rounded-full text-xs border border-neutral-400/20 hover:bg-brand-primary hover:text-white transition-all"
                >
                  {q}
                </button>
              ))}
            ...
            <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-2">
              <div className="flex-1 relative">
                <input
                  ...
                  disabled={isLoading}
                  className="w-full px-4 py-3 bg-neutral-300 text-neutral-700 border-none rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20 disabled:opacity-50 pr-24"
                />
                <div className="absolute right-2 bottom-1.5 flex items-center gap-1">
                  <button
                    ...
                    className="p-2 text-neutral-500 hover:text-brand-primary hover:bg-green-50 rounded-lg transition-colors"
