  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-neutral-600 mt-1 flex items-center gap-2">
            <span className="inline-flex items-center justify-center bg-brand-accent/20 text-yellow-800 px-2 py-0.5 rounded-full text-xs font-bold">
        ...
        <div className="flex gap-3">
          ...
          <button className="bg-brand-primary text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-brand-secondary transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-primary/5">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center">
              <MaterialSymbol icon="notifications_active" className="text-blue-600" size={20} />
            </div>
            <span className="font-bold text-brand-primary">{t('alertsPage.notify')}</span>
          </div>
          <p className="text-xs text-neutral-600">{t('alertsPage.notifyDesc')}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-primary/5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="font-bold text-brand-primary">{t('alertsPage.resolve')}</span>
          </div>
          <p className="text-xs text-neutral-600">{t('alertsPage.resolveDesc')}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-primary/5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="font-bold text-brand-primary">{t('alertsPage.deactivateAllDesc')}</span>
          </div>
          <p className="text-xs text-neutral-600">{t('alertsPage.deactivateAllDescText')}</p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-primary/5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="font-bold text-brand-primary">{t('common.delete')}</span>
          </div>
          <p className="text-xs text-neutral-600">{t('alertsPage.deleteDesc')}</p>
        </div>
      </div>

      {/* Bento Filter Bar */}
      <div className="bg-neutral-100 p-2 rounded-2xl flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl shadow-sm">
          ...
          <button 
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-colors ${typeFilter === 'all' ? 'bg-brand-primary text-white shadow-md' : 'text-stone-600 hover:bg-stone-100'}`}
          >
            {t('common.all')}
          </button>
          <button 
            onClick={() => setTypeFilter('geofence')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${typeFilter === 'geofence' ? 'bg-brand-primary text-white' : 'text-stone-600 hover:bg-stone-100'}`}
          >
            {t('alertsPage.geofence')}
          </button>
          <button 
            onClick={() => setTypeFilter('temperature')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${typeFilter === 'temperature' ? 'bg-brand-primary text-white' : 'text-stone-600 hover:bg-stone-100'}`}
          >
            {t('alertsPage.temperature')}
          </button>
          <button 
            onClick={() => setTypeFilter('offline')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${typeFilter === 'offline' ? 'bg-brand-primary text-white' : 'text-stone-600 hover:bg-stone-100'}`}
          >
        ...
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl shadow-sm">
          ...
          <button 
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-colors ${statusFilter === 'all' ? 'bg-green-100 text-brand-primary' : 'text-stone-600 hover:bg-stone-100'}`}
          >
            {t('common.all')}
          </button>
          <button 
            onClick={() => setStatusFilter('unresolved')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${statusFilter === 'unresolved' ? 'bg-green-100 text-brand-primary' : 'text-stone-600 hover:bg-stone-100'}`}
          >
            {t('alertsPage.unresolved')}
          </button>
          <button 
            onClick={() => setStatusFilter('resolved')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${statusFilter === 'resolved' ? 'bg-green-100 text-brand-primary' : 'text-stone-600 hover:bg-stone-100'}`}
          >
      ...
      <div className="bg-white rounded-3xl overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <thead className="bg-neutral-100/50 border-b border-neutral-300">
            <tr>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600">{t('alertsPage.alertType')}</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600">{t('alertsPage.animalId')}</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600">{t('alertsPage.location')}</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600">{t('alertsPage.timestamp')}</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600">{t('alertsPage.severity')}</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600">{t('common.status')}</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-neutral-600 text-right">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-200/50">
            {loading ? (
              ...
              return (
                <tr key={alert.id} className="group hover:bg-neutral-100/30 transition-colors">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-neutral-500">{alert.geofence?.name || t('alertsPage.unknownZone')}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    {alert.animal ? (
                      <Link to={`/animals/${alert.animal.id}`} className="font-bold text-sm text-brand-primary hover:underline">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {!alert.is_acknowledged && (
                        <>
                          ...
                          <button 
                            ...
                            className="bg-brand-primary text-white text-[10px] font-bold px-3 py-1.5 rounded-lg hover:bg-brand-secondary transition-colors"
                          >
                      ...
                      <Link 
                        to="/map"
                        className="border border-neutral-400 text-neutral-600 text-[10px] font-bold px-3 py-1.5 rounded-lg hover:bg-neutral-300 transition-colors"
                      >
                        {t('common.view')}
                      </Link>
                      <button 
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-1 bg-brand-secondary text-white p-6 rounded-3xl relative overflow-hidden group">
          <div className="relative z-10">
            <MaterialSymbol icon="verified" className="text-brand-accent text-4xl mb-4" />
            ...
            <div className="mt-6 flex items-end gap-2">
              ...
              <span className="text-brand-accent font-bold text-xs pb-1">+2.1% ↑</span>
            </div>
          </div>
        ...
        <div className="col-span-2 bg-white border border-neutral-300 p-6 rounded-3xl flex gap-8">
          <div className="w-2/3 h-full flex flex-col justify-between">
            <div>
              <h3 className="text-lg font-black text-brand-primary font-['Manrope']">{t('alertsPage.geofenceHotspots')}</h3>
              <p className="text-neutral-500 text-xs mt-1">{t('alertsPage.breachLocations')}</p>
            </div>
            <div className="space-y-3 mt-4">
              <div className="flex items-center gap-4">
                <span className="text-[10px] font-bold text-neutral-500 w-20">NORTH GATE</span>
                ...
                <span className="text-xs font-black text-brand-primary">85%</span>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-[10px] font-bold text-neutral-500 w-20">WATERING HOLE</span>
                <div className="flex-1 h-2 bg-neutral-300 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-primary w-[12%] rounded-full"></div>
                </div>
                <span className="text-xs font-black text-brand-primary">12%</span>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-[10px] font-bold text-neutral-500 w-20">EAST SECTOR</span>
                <div className="flex-1 h-2 bg-neutral-300 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-primary w-[3%] rounded-full"></div>
                </div>
                <span className="text-xs font-black text-brand-primary">3%</span>
              </div>
            </div>
          </div>
          <div className="w-1/3 h-full rounded-2xl overflow-hidden relative border border-neutral-300">
            <div className="w-full h-full bg-gradient-to-br from-emerald-100 to-emerald-200 flex items-center justify-center">
              <MaterialSymbol icon="map" size={48} className="text-emerald-300" />
            </div>
            <Link to="/map" className="absolute inset-0 bg-brand-primary/20 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
              <button className="bg-white/90 backdrop-blur-sm text-brand-primary text-[10px] font-black px-3 py-2 rounded-lg shadow-lg">
