    ...
    <div className="space-y-8">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-neutral-600 mt-1">{t('medicalRecords.subtitle') || 'Track and manage herd medical history'}</p>
        </div>
        <div className="flex gap-2">
          <div className="flex bg-neutral-300 p-1 rounded-xl">
            <button
      ...
      {activeTab === 'records' ? (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="bg-white rounded-xl p-4 shadow-sm">
            <p className="text-xs text-neutral-500 uppercase tracking-wider font-bold">{t('medicalRecords.total') || 'Total Records'}</p>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-xl p-4 shadow-sm">
              <p className="text-xs text-neutral-500 uppercase tracking-wider font-bold">{t('vaccination.total')}</p>
              <p className="text-2xl font-black text-brand-primary">{vaccinations.length}</p>
            </div>
            <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
            ...
            <>
              <div className="flex items-center justify-between">
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={goToToday}
                    className="px-4 py-2 text-sm bg-brand-primary text-white rounded-lg font-medium hover:bg-brand-secondary transition-colors ml-2"
                  >
                    {t('vaccination.today')}
                  </button>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-4 py-2 text-sm rounded-lg font-medium transition-colors ${
                      activeView === 'calendar' ? 'bg-brand-primary text-white' : 'bg-neutral-300 text-neutral-600 hover:bg-neutral-100'
                    }`}
                  >
                    {t('vaccination.month')}
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-sm rounded-lg font-medium transition-colors ${
                      activeView === 'table' ? 'bg-brand-primary text-white' : 'bg-neutral-300 text-neutral-600 hover:bg-neutral-100'
                    }`}
                  >
                    {t('common.list')}
                  </button>
              ...
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-4">
                <div className="grid grid-cols-7 bg-brand-primary text-white text-center py-4">
                ...
                <div className="grid grid-cols-7">
                  {calendarDays.map(({ date, isCurrentMonth }, index) => {
                    ...
                    const bgClass = isCurrentMonth ? 'bg-white' : 'bg-neutral-100';
                    const todayClass = isToday ? 'bg-brand-accent/10' : '';
                    const spanClass = isToday ? 'bg-brand-accent text-white px-2 py-0.5 rounded-full' : isCurrentMonth ? 'text-neutral-600' : 'text-neutral-600/40';
                    
                    return (
                      <div
                        key={index}
                        className={`p-3 min-h-[100px] border-r border-b border-neutral-100 ${bgClass} ${todayClass}`}
                      >
                        ...
                        <div className="mt-1 space-y-1">
                          ...
                          {dayVaccinations.length > 2 && (
                            <div className="text-[10px] text-neutral-600 font-medium">+{dayVaccinations.length - 2}</div>
                          )}
                        </div>
                      </div>
                    );
      ...
      {/* Filters */}
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-neutral-200">
        <div className={`flex flex-wrap items-center gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
          <div className={`flex items-center gap-2 px-4 py-2 bg-neutral-100 rounded-lg ${isRtl ? 'flex-row-reverse' : ''}`}>
      ...
      <div className="bg-white rounded-[2rem] overflow-hidden shadow-sm">
        <div className="p-6 border-b border-neutral-200 flex justify-between items-center">
          <h3 className="text-xl font-bold text-brand-primary">{activeTab === 'records' ? t('medicalRecords.recentRecords') : t('vaccination.schedules')}</h3>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-neutral-100">
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('common.date')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('nav.animals')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('common.type')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('medicalRecords.title') || 'Title'}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('medicalRecords.veterinarian')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('common.status')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm text-right">{t('common.actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-200">
              {records.length === 0 ? (
                ...
                records.map((record) => (
                  <tr key={record.id} className="hover:bg-neutral-100/50 transition-colors">
                    <td className="px-6 py-5 font-medium text-brand-primary">
                      {new Date(record.record_date).toLocaleDateString()}
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center text-brand-primary font-black text-xs">
                    ...
                    <td className="px-6 py-5 text-neutral-600">{record.title}</td>
                    <td className="px-6 py-5 text-neutral-600">{record.veterinarian || '-'}</td>
                    <td className="px-6 py-5">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-neutral-100">
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('common.date')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('nav.animals')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('vaccination.vaccine')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('vaccination.vaccinationType')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('medicalRecords.veterinarian')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm">{t('common.status')}</th>
                <th className="px-6 py-4 text-green-600 font-bold text-sm text-right">{t('common.actions')}</th>
              </tr>
            ...
            <tbody className="divide-y divide-neutral-200">
              {vaccinations.length === 0 ? (
                ...
                vaccinations.map((vacc) => (
                  <tr key={vacc.id} className="hover:bg-neutral-100/50 transition-colors">
                    <td className="px-6 py-5 font-medium text-brand-primary">
                    ...
                    <td className="px-6 py-5 text-neutral-600">{vacc.vaccine_name}</td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-neutral-600">{vacc.veterinarian || '-'}</td>
                    <td className="px-6 py-5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('nav.animals')} *</label>
                <select
                  value={formData.animal_id}
                  onChange={(e) => setFormData({ ...formData, animal_id: e.target.value })}
                  className="w-full bg-neutral-100 rounded-xl p-3 text-brand-primary focus:ring-2 focus:ring-brand-secondary/20"
                  required
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                {activeTab === 'vaccinations' ? (
                  <>
                    <div>
                      <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('vaccination.vaccine')} *</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('vaccination.vaccinationType')}</label>
                      <select
                  ...
                  <>
                    <div>
                      <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.recordType')} *</label>
                      <select
                    ...
                    <div>
                      <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('common.status')}</label>
                      <select
              ...
              {activeTab !== 'vaccinations' && (
                <div>
                  <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.title')} *</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('common.date')} *</label>
                <input
                  type="date"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.veterinarian')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.medication')}</label>
                  <input
                    type="text"
              ...
              {activeTab !== 'vaccinations' && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.dosage')}</label>
                    <input
                      type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.description')}</label>
                <textarea
                  value={formData.description}
                  onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  className="w-full bg-neutral-100 rounded-xl p-3 text-brand-primary focus:ring-2 focus:ring-brand-secondary/20 h-20 resize-none"
                  placeholder="Medical notes..."
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-neutral-600 uppercase mb-2">{t('medicalRecords.nextFollowUp')}</label>
                <input
                  type="date"
              ...
              <div className="flex gap-4 pt-4">
                <button
                  ...
                  className="flex-1 py-3 bg-neutral-100 text-brand-primary rounded-xl font-bold hover:bg-neutral-200 transition"
                >
                  {t('common.cancel')}
                </button>
                <button
