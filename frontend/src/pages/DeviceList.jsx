...
const statusConfig = {
  ...
  low_signal: { icon: 'signal_cellular_alt_1_bar', label: 'Low Signal', color: 'text-yellow-800' },
  offline: { icon: 'wifi_off', label: 'Offline', color: 'text-neutral-500' },
};
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm">
          <span className="text-neutral-500 text-xs font-bold uppercase">{t('devices.title')}</span>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border-l-4 border-brand-primary">
          <span className="text-neutral-500 text-xs font-bold uppercase">{t('devicesPage.onlineStatus')}</span>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border-l-4 border-yellow-800">
          <span className="text-neutral-500 text-xs font-bold uppercase">{t('devicesPage.lowBatteryAlerts')}</span>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-3xl font-black text-yellow-800">{lowBatteryCount}</span>
          </div>
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm">
          <span className="text-neutral-500 text-xs font-bold uppercase">{t('devicesPage.maintenanceDue')}</span>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-3xl font-black text-green-600">{maintenanceCount}</span>
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
              filter === status
                ...
                : 'bg-white text-neutral-600 border border-neutral-400/20 hover:bg-neutral-100'
            }`}
          >
      ...
      {/* Device Table */}
      <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-neutral-400/10">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-neutral-300/50 text-neutral-600 text-xs uppercase tracking-widest font-bold">
            ...
            <tbody className="divide-y divide-neutral-400/10">
              {filteredDevices.map((device) => {
                ...
                return (
                  <tr key={device.id} className={`hover:bg-neutral-100/50 transition-colors group ${battery === 0 ? 'bg-red-50/5' : ''}`}>
                    <td className="px-8 py-5">
                      <div className="font-['Manrope'] font-bold text-brand-primary">{device.device_id}</div>
                      <div className="text-[10px] text-neutral-600">{device.firmware_version || 'v2.4'} {t('devicesPage.firmware')}</div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="font-medium text-brand-primary">{device.name || '-'}</div>
                      <div className="text-[10px] text-neutral-600">{device.type || '-'}</div>
                    </td>
                    <td className="px-8 py-5">
                      {device.owner ? (
                        <div className="font-medium text-brand-primary">{device.owner.name}</div>
                      ) : (
                      ...
                      {(() => {
                        ...
                        return assignedAnimal ? (
                          <Link
                            ...
                            className="flex items-center gap-3 hover:bg-neutral-100 rounded-lg p-2 -m-2 transition-colors"
                          >
                            <div className="w-10 h-10 rounded-full bg-neutral-300 flex items-center justify-center">
                              <MaterialSymbol icon="pets" size={20} className="text-brand-primary" />
                            </div>
                            <div>
                              <div className="font-bold text-sm text-brand-primary hover:text-brand-secondary">{assignedAnimal.animal_id}</div>
                              <div className="text-xs text-neutral-600">{assignedAnimal.species}</div>
                            </div>
                          </Link>
                        ) : (
                          <div>
                            <div className="font-bold text-sm text-neutral-600">{t('devicesPage.unassigned')}</div>
                            <div className="text-xs text-neutral-600/60">{t('devicesPage.noAssociation')}</div>
                          </div>
                        );
                      })()}
                    </td>
                        ...
                        <div className="w-32">
                          <div className="flex justify-between text-xs font-bold mb-1.5">
                            <span className={
                              ...
                              battery > 20 ? 'text-yellow-800' : 'text-danger'
                            }>{battery}%</span>
                        ...
                        <div className="h-2 w-full bg-neutral-100 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${
                              ...
                              battery > 20 ? 'bg-yellow-800' : 'bg-neutral-700'
                            }`}
                            style={{ width: `${battery}%` }}
                          />
                        </div>
                    ...
                    <td className="px-8 py-5 text-sm text-neutral-600 font-medium">
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link
                          ...
                          className="p-2 hover:bg-neutral-100 rounded-lg text-neutral-600 transition-all"
                        >
        ...
        {filteredDevices.length === 0 && (
          <div className="p-12 text-center">
            <MaterialSymbol icon="sensors_off" size={48} className="mx-auto text-neutral-500 mb-4" />
            <p className="text-neutral-500">{t('devicesPage.noDevices')}</p>
          </div>
        )}
