...
export default function TasksPage() {
  ...
  const priorityColors = {
    ...
    medium: 'bg-brand-accent/15 text-yellow-800',
    high: 'bg-amber-500/15 text-yellow-800',
    urgent: 'bg-danger/10 text-danger',
  };

  const statusColors = {
    pending: 'bg-amber-500/15 text-yellow-800',
    ...
    <div className="space-y-8">
      <div className={`flex flex-col md:flex-row md:items-end justify-between gap-4 ${isRtl ? 'text-right' : ''}`}>
        <div>
          ...
          <p className="text-neutral-600 mt-2 font-medium">Assign and track shepherd activities</p>
        </div>
        <div className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
          <Link 
            to="/task-logs-archive"
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-neutral-100 text-brand-primary font-semibold hover:bg-neutral-200 transition-colors"
          >
      ...
      {message && (
        <div className={`p-4 rounded-xl ${message.type === 'success' ? 'bg-emerald-500/15 text-success' : 'bg-danger/10 text-danger'}`}>
          {message.text}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: t('tasks.pending'), value: stats.pending, color: 'bg-amber-500/5' },
          { label: t('tasks.inProgress'), value: stats.in_progress, color: 'bg-blue-500/5' },
          { label: t('tasks.completed'), value: stats.completed, color: 'bg-emerald-500/5' },
      ...
      <div className={`flex flex-wrap gap-3 items-center ${isRtl ? 'flex-row-reverse' : ''}`}>
        <div className={`flex items-center gap-3 bg-white px-4 py-2.5 rounded-xl ${isRtl ? 'flex-row-reverse' : ''}`}>
          <MaterialSymbol icon="filter_list" size={20} className="text-neutral-500" />
          <select
            value={statusFilter}
        ...
        <div className={`flex items-center gap-3 bg-white px-4 py-2.5 rounded-xl ${isRtl ? 'flex-row-reverse' : ''}`}>
          <MaterialSymbol icon="flag" size={20} className="text-neutral-500" />
          <select
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          <MaterialSymbol icon="task_alt" size={64} className="text-neutral-500 mx-auto mb-4 opacity-50" />
          <p className="text-neutral-600 font-medium text-lg">{t('common.noData')}</p>
          <p className="text-neutral-500 text-sm mt-1">Create a task to assign work to shepherds</p>
        </div>
      ) : (
        <div className="space-y-4">
          {tasks.map((task) => (
            <div key={task.id} className={`card overflow-hidden ${isOverdue(task) ? 'ring-2 ring-danger/30' : ''}`}>
              <div 
                className="p-6 cursor-pointer hover:bg-neutral-100/30 transition-colors"
                onClick={() => toggleTaskExpansion(task.id)}
              >
                <div className={`flex items-start gap-5 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${task.status === 'completed' ? 'bg-emerald-500/15' : 'bg-brand-primary/5'}`}>
                    <MaterialSymbol 
                  ...
                  <div className="flex-1">
                    ...
                    <div className={`flex flex-wrap items-center gap-5 mt-4 text-sm text-neutral-500 ${isRtl ? 'flex-row-reverse justify-end' : ''}`}>
                      {task.assignee && (
                        <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    {canSubmitLog(task) && (
                      <button
                        ...
                        className="p-3 bg-brand-accent/15 text-yellow-800 hover:bg-brand-accent/25 rounded-xl transition-colors"
                    ...
                    {task.status === 'pending' && !isShepherd && (
                      <button
                        ...
                        className="p-3 bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 rounded-xl transition-colors"
                        title="Start Task"
                      >
                    ...
                    {task.status === 'in_progress' && (
                      <button
                        ...
                        className="p-3 bg-emerald-500/10 text-success hover:bg-emerald-500/20 rounded-xl transition-colors"
                    ...
                    {task.status !== 'completed' && task.status !== 'cancelled' && canModify && (
                      <button
                        ...
                        className="p-3 text-neutral-500 hover:bg-neutral-100 rounded-xl transition-colors"
              ...
              {expandedTask === task.id && (
                <div className="border-t border-neutral-100 bg-neutral-100/30 p-6">
                  ...
                  {taskLogs[task.id]?.length > 0 ? (
                    <div className="space-y-3">
                      {taskLogs[task.id].map((log) => (
                        <div key={log.id} className={`bg-white p-4 rounded-xl ${isRtl ? 'text-right' : ''}`}>
                          <div className={`flex items-start justify-between ${isRtl ? 'flex-row-reverse' : ''}`}>
                            <div className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
                              ...
                              <div>
                                ...
                                <p className="text-sm text-neutral-500">{log.user?.name}</p>
                              </div>
                            </div>
                          ...
                          <p className="mt-3 text-sm text-neutral-600">{log.description}</p>
                          {log.photo_path && (
                            <img 
                    ...
                    <p className="text-neutral-500 text-sm italic">{t('tasks.noLogs')}</p>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className={`flex items-center justify-between mb-8 ${isRtl ? 'flex-row-reverse' : ''}`}>
              ...
              <button onClick={() => setShowCreateModal(false)} className="p-3 hover:bg-neutral-100 rounded-xl transition">
                <MaterialSymbol icon="close" size={24} className="text-neutral-500" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateTask} className="space-y-6">
              {canModify && (
                <div>
                  <label className={`block text-xs font-bold text-neutral-600 uppercase tracking-wider mb-2 ${isRtl ? 'text-right' : ''}`}>
              ...
              <div className="border-t border-neutral-100 pt-5">
                ...
                {recurring.enabled && (
                  <div className="mt-4 p-4 bg-neutral-100/50 rounded-xl space-y-4">
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-sm text-neutral-500">{t('tasks.daysUnit')}</span>
                        </div>
                      </div>
              ...
              <div className={`flex gap-4 pt-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                <button
                  ...
                  className="flex-1 py-3 bg-neutral-100 text-brand-primary rounded-xl font-bold text-sm hover:bg-neutral-200 transition"
                >
                  {t('common.cancel')}
                </button>
                <button
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl">
            <div className={`flex items-center justify-between mb-8 ${isRtl ? 'flex-row-reverse' : ''}`}>
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">{selectedTask.title}</p>
              </div>
              <button onClick={() => setShowLogModal(false)} className="p-3 hover:bg-neutral-100 rounded-xl transition">
                <MaterialSymbol icon="close" size={24} className="text-neutral-500" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmitLog} className="space-y-6">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-neutral-100 rounded-xl py-3 px-4 text-sm font-medium text-brand-primary transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-secondary/20"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-neutral-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-neutral-100 file:text-brand-primary hover:file:bg-neutral-200"
                />
              </div>
