  ...
  return (
    <div className="space-y-8">
      {(isFromRegistration || isNewUser) && currentSubscription?.status !== 'active' && currentSubscription?.status !== 'pending_payment' && (
        <div className="bg-gradient-to-r from-brand-accent/20 to-brand-primary/10 border border-brand-accent rounded-2xl p-6 text-center">
          <MaterialSymbol icon="celebration" size={48} className="text-brand-accent mx-auto mb-3" weight="fill" />
          <h2 className="text-2xl font-bold text-brand-primary mb-2">Welcome to The Oasis!</h2>
      ...
      <div>
        <h2 className="text-3xl font-bold text-brand-primary">Subscription Plans</h2>
        <p className="text-neutral-600 mt-1">Choose the perfect plan for your livestock management needs</p>
      </div>
      ...
      {limits && currentSubscription?.status === 'active' && (
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <h3 className="font-bold text-brand-primary mb-4">Current Usage</h3>
          ...
          return (
            <div
              ...
              className={`bg-white rounded-2xl p-6 shadow-sm border-2 ${
                isCurrent ? 'border-brand-accent' : 'border-transparent'
              }`}
            >
              {isCurrent && (
                <div className="bg-brand-accent text-white text-xs font-bold px-3 py-1 rounded-full inline-block mb-3">
                  Current Plan
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-neutral-100 rounded-xl p-4 mb-6">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-brand-primary">{selectedTier.name}</span>
              </div>
              <div className="flex justify-between mt-2">
                <span className="text-neutral-500">Amount</span>
                <span className="font-bold text-brand-accent">{formatPrice(selectedTier.price_monthly)}/mo</span>
              </div>
            </div>
            ...
            {(selectedTier.price_monthly === '0.00' || selectedTier.price_monthly === 0) ? (
              ...
              <>
                <div className="flex gap-2 mb-6">
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-medium transition ${
                      paymentMethod === 'card' ? 'bg-brand-primary text-white' : 'bg-gray-100 text-neutral-600'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-medium transition ${
                      paymentMethod === 'bank' ? 'bg-brand-primary text-white' : 'bg-gray-100 text-neutral-600'
                    }`}
                  >
                ...
                {paymentMethod === 'card' ? (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-neutral-600 mb-1">Card Number</label>
                      <input
                        type="text"
                        value={cardNumber}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-neutral-600 mb-1">Expiry</label>
                        <input
                          type="text"
                          value={expiry}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-neutral-600 mb-1">CVC</label>
                        <input
                          type="text"
                          value={cvc}
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-neutral-600 mb-2">Upload Payment Proof (PDF)</label>
                      <input
                        type="file"
                        accept=".pdf"
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-gray-100 text-neutral-600 rounded-xl font-medium hover:bg-gray-200 transition"
                    >
                      Cancel
                    </button>
                  </div>
