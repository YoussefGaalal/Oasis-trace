...
export default function MapView() {
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-8rem)] flex items-center justify-center bg-brand-light">
        <div className="text-center">
          <div className="animate-spin w-12 h-12 border-4 border-brand-primary border-t-transparent rounded-full mx-auto mb-4" />
          <p className="text-neutral-600 font-medium">{t('mapPage.loadingMap')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100vh-8rem)] relative overflow-hidden bg-brand-light">
      {/* Header */}
      <header className="absolute top-0 left-0 right-0 z-[1000] flex justify-between items-center px-4 py-2 bg-brand-primary/95 backdrop-blur-md shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div className="relative hidden lg:block">
            <input
              ...
              className="w-48 bg-white/10 border-none rounded-lg pl-10 pr-4 py-1.5 text-white text-sm placeholder:text-white/50 focus:ring-2 focus:ring-brand-accent focus:bg-white/15 transition-all"
            />
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${showLegend ? 'bg-brand-accent text-brand-primary' : 'bg-white/10 text-white hover:bg-white/20'}`}
                  ...
                  <div className="p-3 min-w-[220px]">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <div className="bg-gray-50 rounded-lg p-2">
                        <div className="text-[10px] text-neutral-500 uppercase">Perimeter</div>
                        <div className="text-sm font-bold text-brand-primary">{perimeterKm} km</div>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-2">
                        <div className="text-[10px] text-neutral-500 uppercase">Alert</div>
                        <div className="text-sm font-bold text-brand-primary capitalize">{geofence.alert_type}</div>
                      </div>
                      <div className="bg-green-50 rounded-lg p-2">
                        <div className="text-[10px] text-neutral-500 uppercase">Inside</div>
                        <div className="text-sm font-bold text-green-600">{stats.animalsInside || 0}</div>
                      </div>
                      <div className="bg-amber-50 rounded-lg p-2">
                        <div className="text-[10px] text-neutral-500 uppercase">Outside</div>
                ...
                <Popup>
                  <div className="p-3 min-w-[200px]">
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-brand-primary">{animal.animal_id}</h3>
                    ...
                    <p className="text-xs text-neutral-600 mt-1">{animal.species} {animal.breed && `• ${animal.breed}`}</p>
                    <p className="text-xs text-neutral-500 mt-1">Last: {formatLastSeen(getLastLocationTime(animal.id))}</p>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-gray-50 p-2 rounded">
                        <span className="text-neutral-500">Temp</span>
                        <p className={`font-bold ${getAnimalStatus(animal) === 'critical' ? 'text-red-600' : 'text-brand-primary'}`}>
                          {animal.baseline_temperature || 'N/A'}°C
                        </p>
                      </div>
                      <div className="bg-gray-50 p-2 rounded">
                        <span className="text-neutral-500">Points</span>
                        <p className="font-bold text-brand-primary">{getPathPositions(animal.id).length || (getLastLocation(animal.id) ? 1 : 0)}</p>
                      </div>
                    </div>
      ...
      {showLegend && (
        <div className="absolute top-24 right-6 z-[1000] bg-white rounded-xl shadow-xl p-4 min-w-[200px]">
          ...
          <div className="space-y-2">
            <div className="text-xs font-semibold text-neutral-500 uppercase mb-1">{t('mapPage.animalGroups')}</div>
            ...
            <div className="text-xs font-semibold text-neutral-500 uppercase mb-1">{t('mapPage.pathSelection')}</div>
            <div className="flex items-center gap-2 text-xs">
            ...
            <div className="border-t border-gray-100 my-2" />
            <div className="text-xs font-semibold text-neutral-500 uppercase mb-1">{t('mapPage.fenceArea')}</div>
            <div className="flex items-center gap-2 text-xs">
              <span className="w-4 h-4 rounded bg-brand-accent/30 border-2 border-brand-accent" />
      ...
      <div className="absolute left-4 top-24 z-[1000] flex flex-col gap-2">
        ...
        {isDrawingGeofence && (
          <div className="bg-white p-3 rounded-xl shadow-lg text-xs text-neutral-600 max-w-[220px] animate-in fade-in">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              <div
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all ${
                  selectedAnimal?.id === animal.id 
                    ...
                    : 'border-neutral-300 hover:border-brand-primary/20 hover:shadow-sm'
                }`}
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <span className={`w-2.5 h-2.5 rounded-full ${isOnline(animal.id) ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`} />
                    <h4 className="font-bold text-brand-primary text-sm">{animal.animal_id}</h4>
                  </div>
                ...
                <p className="text-xs text-neutral-500 mb-2">{animal.species} {animal.breed && `• ${animal.breed}`}</p>
                <p className="text-[10px] text-neutral-500 mb-3">Last: {formatLastSeen(getLastLocationTime(animal.id))}</p>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-gray-50 rounded-lg p-2">
                    <MaterialSymbol icon="thermostat" size={14} className={getAnimalStatus(animal) === 'critical' ? 'text-red-500' : 'text-neutral-500'} />
                  ...
                  <div className="bg-gray-50 rounded-lg p-2">
                    <MaterialSymbol icon="location_on" size={14} className="text-neutral-500" />
                    <p className="text-xs font-bold mt-1 text-brand-primary">{getPathPositions(animal.id).length}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-2">
                    <MaterialSymbol icon="schedule" size={14} className="text-neutral-500" />
                    <p className="text-xs font-bold mt-1 text-brand-primary">{timeFilter}h</p>
                  </div>
                </div>
            ...
            {filteredAnimalsWithoutPaths.length > 0 && (
              <div className="pt-4 border-t border-neutral-300">
                <p className="text-xs text-neutral-500 mb-3 font-medium flex items-center gap-2">
                  ...
                  <div
                    ...
                    className={`p-3 rounded-xl border cursor-pointer transition-all mb-2 ${
                      selectedAnimal?.id === animal.id 
                        ...
                        : 'border-neutral-300 hover:border-brand-primary/20'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full bg-gray-300" />
                      <span className="font-medium text-sm text-neutral-600">{animal.animal_id}</span>
                    </div>
                    <p className="text-xs text-neutral-500 ml-4">{animal.species}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {showAlertsPanel && (
        <div className="absolute top-24 left-4 w-80 bg-white rounded-2xl shadow-2xl overflow-hidden z-[1000] max-h-[50vh]">
          <div className="p-4 border-b border-neutral-300 bg-gradient-to-r from-amber-500 to-amber-600">
            <div className="flex justify-between items-center">
          ...
          <div className="divide-y divide-neutral-200 max-h-[35vh] overflow-y-auto">
            {unacknowledgedAlerts.length === 0 ? (
                ...
                <div key={alert.id} className={`p-4 ${alert.is_acknowledged ? 'opacity-50' : ''}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-neutral-500">{alert.geofence?.name || 'Geofence'}</p>
                      <p className="text-xs text-neutral-500">{new Date(alert.triggered_at).toLocaleString()}</p>
                    </div>
          ...
          <div className="bg-white rounded-2xl p-6 w-96 shadow-2xl animate-in zoom-in-95 fade-in duration-200">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-neutral-300 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-accent"
                  autoFocus
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Alert Type</label>
                <select 
                  value={newGeofenceAlertType}
                  onChange={(e) => setNewGeofenceAlertType(e.target.value)}
                  className="w-full bg-neutral-300 border-none rounded-xl px-4 py-3 text-sm"
                >
            ...
            <div className="flex gap-3 mt-6">
              <button 
                ...
                className="flex-1 py-3 bg-neutral-300 text-brand-primary rounded-xl font-bold text-sm hover:bg-gray-200 transition-colors"
              >
                Cancel
              </button>
              <button 
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[1000]">
        <div className="bg-white/95 backdrop-blur-md rounded-full px-5 py-2 shadow-xl flex items-center gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-neutral-600">{filteredAnimalsWithoutPaths.length} untracked</span>
          </div>
          <div className="h-4 w-px bg-gray-200" />
          <div className="flex items-center gap-2" title="Active Geofences">
            <span className="w-2 h-2 bg-brand-accent rounded-sm" />
            <span className="text-sm text-neutral-500">{geofences.filter(g => g.is_active).length} fences</span>
          </div>
          {selectedGeofence !== 'all' && geofenceStats[selectedGeofence] && (
            <>
              ...
              <div className="flex items-center gap-1 text-xs" title="Selected fence perimeter">
                ...
                <span className="text-neutral-500">{geofenceStats[selectedGeofence].perimeter?.toFixed(1) || '-'} km</span>
              </div>
            </>
          )}
          <div className="h-4 w-px bg-gray-200" />
