  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Link to="/auctions" className="p-2 hover:bg-neutral-300 rounded-full transition-colors">
        ...
        <div>
          ...
          <p className="text-neutral-600 mt-1">{t('auctionsPage.listForBidding', 'List your animal for bidding')}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {/* Select Animal */}
        <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-neutral-400/10">
          <div className="flex items-center gap-3 mb-6">
            <span className="material-symbols-outlined text-brand-primary bg-neutral-100 p-2 rounded-xl">pets</span>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-neutral-100 rounded-2xl">
              <MaterialSymbol icon="pets" size={48} className="mx-auto text-neutral-400 mb-4" />
              <p className="text-neutral-600 font-semibold">{t('auctionsPage.noAnimalsAvailable')}</p>
              <p className="text-neutral-500 text-sm mt-2">{t('auctionsPage.noAnimalsHint')}</p>
                ...
                <div
                  ...
                  className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-4 ${
                    parseInt(formData.animal_id) === animal.id
                      ? 'border-brand-primary bg-neutral-100'
                      : 'border-neutral-300 hover:border-brand-primary/30'
                  }`}
                >
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-neutral-300 flex-shrink-0">
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-sm text-neutral-600">{animal.species} {animal.breed && `• ${animal.breed}`}</p>
                    <p className="text-xs text-neutral-500">{animal.gender} | Temp: {animal.baseline_temperature || '38.5'}°C</p>
                  </div>
                  {parseInt(formData.animal_id) === animal.id && (
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-neutral-400/10">
          <div className="flex items-center gap-3 mb-6">
            <span className="material-symbols-outlined text-brand-primary bg-neutral-100 p-2 rounded-xl">gavel</span>
            <h3 className="text-xl font-bold font-['Manrope'] text-brand-primary">{t('auctionsPage.auctionDetails')}</h3>
          </div>

          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-neutral-600/70">{t('auctionsPage.auctionTitle')}</label>
              <input
                name="title"
                ...
                className="w-full bg-neutral-100 border-none rounded-xl px-4 py-3.5 text-brand-primary font-semibold focus:ring-2 focus:ring-brand-primary/20"
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-neutral-600/70">{t('auctionsPage.startingPriceLabel')}</label>
                <input
                  name="starting_price"
                  type="number"
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-neutral-600/70">{t('auctionsPage.reservePriceLabel')}</label>
                <input
                  name="reserve_price"
                  type="number"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-neutral-600/70">{t('auctionsPage.auctionDuration')}</label>
              <select
                name="duration_hours"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-neutral-600/70">{t('auctionsPage.description')} <span className="text-neutral-500 font-normal">Optional</span></label>
              <textarea
                name="description"
                ...
                className="w-full bg-neutral-100 border-none rounded-xl px-4 py-3.5 text-brand-primary font-medium focus:ring-2 focus:ring-brand-primary/20 resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <Link
            to="/auctions"
            className="flex-1 py-4 bg-neutral-300 text-brand-primary rounded-xl font-bold text-center hover:bg-neutral-100 transition-all"
          >
          ...
          <button
            ...
            className="flex-1 py-4 bg-brand-primary text-white rounded-xl font-bold shadow-xl shadow-brand-primary/20 hover:bg-brand-secondary active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {submitting ? (
              <>
