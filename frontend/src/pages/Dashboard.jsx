...
useEffect(() => {
  ...
  const getSeverityClass = (severity) => {
    switch (severity) {
      ...
      case 'Medium': return 'bg-brand-accent/15 text-yellow-800';
    ...
    <div className="space-y-10">
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="stat-card group hover:shadow-[0_16px_48px_rgba(6,64,43,0.1)] transition-shadow duration-300">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-600 mb-1">{t('dashboard.totalAnimals')}</p>
        ...
        <div className="stat-card group hover:shadow-[0_16px_48px_rgba(6,64,43,0.1)] transition-shadow duration-300">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-600 mb-1">{t('dashboard.activeDevices')}</p>
        ...
        <div className="stat-card group hover:shadow-[0_16px_48px_rgba(6,64,43,0.1)] transition-shadow duration-300">
          <div className="flex justify-between items-start mb-6">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-danger/90 to-danger/70 flex items-center justify-center shadow-lg shadow-danger/20">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-600 mb-1">{t('dashboard.alerts')}</p>
        ...
        {stats.subscription?.is_admin ? (
          ...
          <div className="stat-card bg-gradient-to-br from-brand-primary to-brand-secondary text-white group">
            <div className="flex justify-between items-start mb-6">
              <div className="w-14 h-14 rounded-2xl bg-white/15 flex items-center justify-center backdrop-blur-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 card">
          <div className={`p-8 flex justify-between items-center ${isRtl ? 'flex-row-reverse' : ''}`}>
            <h4 className="font-black text-2xl text-brand-primary">
                ...
                >
                  <Popup>
                    <div className="p-3 min-w-[200px]">
                      <h3 className="font-bold text-brand-primary text-lg">{animal.animal_id}</h3>
                      <p className="text-sm text-neutral-600 mt-1">{animal.species}</p>
                      {animal.baseline_temperature && (
                        <p className="text-xs mt-2 text-yellow-800">🌡️ {animal.baseline_temperature}°C</p>
                      )}
                    </div>
                  </Popup>
                </Marker>
                ...
                >
                  <Popup>
                    <div className="p-3">
                      ...
                      <p className="text-xs text-neutral-600">{animal.path.length} tracking points</p>
                    </div>
                  </Popup>
                </Marker>
              ))}
        ...
        <div className="card">
          <div className="p-8">
            <h4 className="font-black text-2xl text-brand-primary mb-6">
                ...
                <div className={`flex items-start gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${getSeverityBg(alert.severity)}`}>
                    <MaterialSymbol 
                      ...
                      size={20}
                      className={alert.severity === 'High' ? 'text-danger' : alert.severity === 'Medium' ? 'text-yellow-800' : 'text-emerald-500'}
                    />
                  </div>
                  <div className="flex-1">
                    <p className="text-xs font-bold uppercase mb-1 opacity-70">{alert.severity}</p>
                    <p className="text-base font-bold text-brand-primary">{alert.device}</p>
      ...
      <div className="card p-8">
        <div className={`flex justify-between items-center mb-6 ${isRtl ? 'flex-row-reverse' : ''}`}>
          <div className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={isRtl ? 'text-right' : ''}>
              <h4 className="font-black text-xl text-brand-primary">{t('vaccination.title')}</h4>
          ...
          <Link to="/medical-records" className="flex items-center gap-2 px-4 py-2 bg-brand-primary text-white rounded-xl font-semibold text-sm hover:bg-brand-secondary transition-colors">
            {t('vaccination.add')}
            <MaterialSymbol icon="arrow_forward" size={16} />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
            <div key={i} className="text-center text-xs font-bold text-neutral-500 py-2">{day}</div>
          ))}
          {(() => {
            ...
            return cells.map((day, idx) => {
              ...
              return (
                <div key={idx} className={`aspect-square rounded-lg flex flex-col items-center justify-center relative ${isToday ? 'bg-brand-accent text-white' : 'bg-neutral-100 hover:bg-neutral-200'} transition-colors cursor-pointer`}>
        ...
        <div className="mt-4 flex items-center justify-center gap-6">
          <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
            ...
            <span className="text-xs text-neutral-500">{t('vaccination.scheduled')}</span>
          </div>
          <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
            <div className="w-3 h-3 bg-red-500 rounded-full" />
            <span className="text-xs text-neutral-500">{t('vaccination.overdue')}</span>
          </div>
        </div>
      </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { to: '/animals', icon: 'pets', title: t('dashboard.manageAnimals'), subtitle: `${stats.totalAnimals} total`, color: 'from-brand-primary to-brand-secondary' },
          { to: '/devices', icon: 'sensors', title: t('nav.devices'), subtitle: `${stats.activeDevices} active`, color: 'from-brand-secondary to-brand-primary' },
          { to: '/map', icon: 'map', title: t('nav.mapView'), subtitle: t('dashboard.fullTracker'), color: 'from-yellow-800 to-brand-accent' },
          { to: '/users', icon: 'groups', title: t('nav.team'), subtitle: t('team.teamMembers'), color: 'from-brand-accent to-yellow-800' },
        ].map((item, idx) => (
          <Link 
            key={idx} 
            to={item.to} 
