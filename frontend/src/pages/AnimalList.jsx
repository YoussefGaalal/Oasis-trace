    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm">
          <p className="text-xs font-bold text-neutral-500 uppercase">{t('animals.title')}</p>
          <p className="text-3xl font-black text-brand-primary mt-1">{totalAnimals}</p>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm">
          <p className="text-xs font-bold text-neutral-500 uppercase">{t('animals.noDeviceAssigned')}</p>
          <p className="text-3xl font-black text-danger mt-1">{unassignedCount}</p>
        </div>
        <div className="bg-brand-accent/10 p-5 rounded-2xl">
          <p className="text-xs font-bold text-yellow-800 uppercase">{t('animals.health')}</p>
          <p className="text-3xl font-black text-yellow-800 mt-1">{assignedCount}</p>
        </div>
      </div>

      {filteredAnimals.length === 0 ? (
        <div className="card p-12 text-center">
          <MaterialSymbol icon="pets" size={64} className="text-neutral-500 mx-auto mb-4 opacity-50" />
          <p className="text-neutral-600 font-medium text-lg">{t('animals.noAnimals')}</p>
        </div>
      ) : (
...
return (
                <div key={animal.id} className="card overflow-hidden group">
                    ...
                    <div className={`flex items-start justify-between mb-4 ${isRtl ? 'flex-row-reverse text-right' : ''}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {animal.name && <p className="text-sm text-neutral-500">{animal.name}</p>}
                          <p className="text-sm text-neutral-500">{animal.species}{animal.breed && ` ${t('common.separator')} ${animal.breed}`}</p>
                        </div>
                      </div>
                  ...
                  {device && (
                    <div className={`flex items-center gap-2 mt-4 p-3 rounded-xl ${device.status === 'offline' ? 'bg-danger/5' : 'bg-emerald-500/5'} ${isRtl ? 'flex-row-reverse' : ''}`}>
                      <MaterialSymbol 
                        icon={device.status === 'offline' ? 'wifi_off' : 'wifi'} 
                        size={16} 
                        className={device.status === 'offline' ? 'text-danger' : 'text-emerald-500'} 
                      />
                      <span className={`text-xs font-medium ${device.status === 'offline' ? 'text-danger' : 'text-emerald-500'}`}>
                        {device.status === 'offline' ? t('devices.offline') : t('devices.online')}
                      </span>
                      {device.battery_level !== undefined && (
                        <span className="text-xs text-neutral-500">{device.battery_level}%</span>
                      )}
                      {device.last_ping && (
                        <span className={`text-xs text-neutral-500 ml-auto ${device.status === 'offline' ? '' : ''}`}>
                ...
                <div className={`flex border-t border-neutral-100 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  <Link 
                    to={`/animals/${animal.id}`}
                    className="flex-1 py-3 text-center text-sm font-semibold text-brand-primary hover:bg-neutral-100 transition-colors"
                  >
                    {t('common.view')}
                  </Link>
                      {canModify && (
                        <>
                          <Link 
                            ...
                            className="flex-1 py-3 text-center text-sm font-semibold text-neutral-550 hover:bg-neutral-100 hover:text-brand-primary transition-colors border-x border-neutral-100"
                          >
                            {t('common.edit')}
                          </Link>
                          {!animalDeviceId && (
                            <button 
                              ...
                              className="flex-1 py-3 text-center text-sm font-semibold text-neutral-550 hover:bg-neutral-100 hover:text-brand-primary transition-colors"
                            >
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
            <div className={`flex items-center justify-between mb-8 ${isRtl ? 'flex-row-reverse' : ''}`}>
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">{selectedAnimal.animal_id}</p>
              </div>
              <button onClick={() => setShowAssignModal(false)} className="p-3 hover:bg-neutral-100 rounded-xl transition">
