  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <nav className="flex text-xs text-green-600 mb-2 uppercase tracking-widest font-bold">
          ...
          <p className="text-neutral-600 mt-1">{auctions.length} {filter === 'active' ? t('auctionsPage.live') : ''} {t('auctionsPage.auctions')}</p>
        </div>
        <Link
          to="/auctions/new"
          className="px-6 py-3 bg-brand-primary text-white rounded-xl font-bold text-sm shadow-lg shadow-brand-primary/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2"
        >
          ...
          <button
            ...
            className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${
              filter === status
                ...
                : 'bg-white text-neutral-600 hover:bg-neutral-300 border border-neutral-400/30'
            }`}
          >
        ...
        <div className="space-y-10">
          {filter === 'all' && myAuctions.length === 0 && enrolledAuctions.length === 0 && (
            <div className="text-center py-16 bg-white rounded-[2rem] shadow-sm">
              <MaterialSymbol icon="gavel" size={64} className="mx-auto text-neutral-400 mb-4" />
              <p className="text-neutral-600 text-lg font-semibold">{t('auctionsPage.noActive')}</p>
              <p className="text-neutral-500 text-sm mt-2">{t('auctionsPage.startNew')}</p>
            </div>
          )}
          ...
          {filter !== 'all' && auctions.length === 0 && (
            <div className="text-center py-16 bg-white rounded-[2rem] shadow-sm">
              <MaterialSymbol icon="gavel" size={64} className="mx-auto text-neutral-400 mb-4" />
              <p className="text-neutral-600 text-lg font-semibold">{t('auctionsPage.noAuctions')}</p>
            </div>
          )}
        </div>
      )}
            ...
            <div className="p-6 space-y-4">
              <div className="bg-neutral-100 rounded-2xl p-4">
                <div className="flex justify-between mb-2">
                  <span className="text-neutral-600 text-sm">{t('auctionsPage.currentPrice')}</span>
                  <span className="font-bold text-neutral-700">{formatPrice(selectedAuction.current_price)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-neutral-600 text-sm">{t('auctionsPage.minimumBid')}</span>
                  <span className="font-bold text-brand-primary">{formatPrice(Math.ceil(selectedAuction.current_price) + 1)}</span>
                </div>
              </div>

              {message && (
                <div className={`p-3 rounded-xl text-sm ${message.type === 'success' ? 'bg-green-100 text-brand-primary' : 'bg-red-50 text-red-800'}`}>
                  {message.text}
                </div>
              )}

              <div>
                <label className="block text-xs font-bold text-neutral-600 uppercase tracking-widest mb-2">{t('auctions.yourBid')} (SAR)</label>
                <input
                  type="number"
                  value={bidAmount}
                  onChange={(e) => setBidAmount(e.target.value)}
                  min={Math.ceil(selectedAuction.current_price) + 1}
                  className="w-full bg-neutral-100 border-none rounded-xl px-4 py-4 text-xl font-bold text-brand-primary focus:ring-2 focus:ring-brand-accent"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 py-3 bg-neutral-300 text-brand-primary rounded-xl font-bold text-sm hover:bg-neutral-100"
                >
                  {t('common.cancel')}
                </button>
                <button
                  ...
                  className="flex-1 py-3 bg-brand-primary text-white rounded-xl font-bold text-sm hover:bg-brand-secondary disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {placing ? (
                    <>
...
function AuctionCard({ auction, onBid }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] overflow-hidden shadow-[0px_12px_32px_rgba(6,64,43,0.06)] group hover:shadow-2xl transition-all duration-500 border border-neutral-400/10">
      ...
      <div className="p-6">
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1">
            <p className="text-[10px] font-black text-yellow-800 uppercase tracking-[0.2em]">
          ...
          <h3 className="text-xl font-bold text-brand-primary font-['Manrope']">{auction.title}</h3>
        ...
        {auction.status === 'sold' && auction.winner ? (
          ...
          <div className="bg-neutral-100 rounded-2xl p-4 mb-4 grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-bold text-green-600 uppercase tracking-widest mb-1">Starting</p>
              <p className="text-lg font-bold text-neutral-700">{formatPrice(auction.starting_price)}</p>
            </div>
            <div>
        ...
        <div className="flex items-center gap-2 mb-6">
          <MaterialSymbol icon="group" size={18} className="text-green-600" />
          <span className="text-xs font-semibold text-neutral-600">
            {auction.bid_count || 0} bids
          </span>
          <span className="mx-2 text-neutral-400">|</span>
          <MaterialSymbol icon="health_and_safety" size={18} className="text-green-600" />
          <span className="text-xs font-semibold text-neutral-600">
        ...
        <div className="flex items-center justify-between">
          <Link
            ...
            className="px-4 py-2 text-brand-primary hover:bg-neutral-300 rounded-xl font-medium text-sm transition-colors"
          >
            View Details
          </Link>
          {auction.status === 'active' && (
